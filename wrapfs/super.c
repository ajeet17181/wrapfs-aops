...
static void wrapfs_put_super(struct super_block *sb)
{
	...
	wrapfs_debug_sbops(WRAPFS_SB(sb)->wrapfs_debug_sb_ops, "");
...
static int wrapfs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	wrapfs_debug_sbops(WRAPFS_SB(dentry->d_sb)->wrapfs_debug_sb_ops, "");
...
static int wrapfs_remount_fs(struct super_block *sb, int *flags, char *options)
{
	...
	wrapfs_debug_sbops(WRAPFS_SB(sb)->wrapfs_debug_sb_ops, "err :%d", err);
	return err;
}
...
static void wrapfs_evict_inode(struct inode *inode)
{
	...
	wrapfs_debug_sbops(WRAPFS_SB(inode->i_sb)->wrapfs_debug_sb_ops, "");
